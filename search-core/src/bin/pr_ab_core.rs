...
fn hash_output(path: &Path) -> u64 {
    let mut files = fs::read_dir(path).unwrap().map(|entry| entry.unwrap().path()).filter(|path| path.is_file()).collect::<Vec<_>>();
    files.sort();
...
fn make_content(file_index: usize, target_bytes: usize) -> Vec<u8> {
    ...
    while text.len() < target_bytes {
        ...
        let _ = writeln!(text, "pub fn module_{module:04}_symbol_{symbol:04}(input: usize) -> usize {{ let value = input.wrapping_mul(31).wrapping_add({value}); value ^ 0x5a5a5a5a }} // file={file_index} line={line}");
        line += 1;
    }
    text.into_bytes()
}
