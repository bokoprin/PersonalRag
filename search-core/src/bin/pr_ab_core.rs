...
fn hash_output(path: &Path) -> u64 {
    ...
    for file in files {
        update_hash(
            &mut hash,
            file.file_name().unwrap().to_string_lossy().as_bytes(),
        );
...
fn temp_output(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "personalrag-ab-core-{label}-{}-{nonce}",
        std::process::id()
    ))
}
...
fn main() {
    ...
    let files = std::env::var("PR_AB_FILES")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(12_000usize);
    let bytes_per_file = std::env::var("PR_AB_BYTES")
        .ok()
        .and_then(|value| value.parse().ok())
    ...
    let report = build_index_unified_benchmark(
        &documents,
        &output,
        &options,
    ...
    println!(
        "AB label={} wall_ms={:.3} files={} bytes_per_file={} workers={} segments={} index_mib={:.3} byte_hash={hash:016x}",
        label,
        wall_ms,
        files,
        bytes_per_file,
        workers,
        report.segments,
        report.index_bytes as f64 / (1024.0 * 1024.0),
    );
