...
use personalrag_gui_bridge_core::{
    ...
    IncrementalSyncResult, IndexBuildPhase, IndexEngine, IndexRequest, IndexResponse,
    PortableEngine, ProductionBackendMode, ProgressRateTracker, RebuildProgress, RebuildStatus,
    SearchBackendStatus, SearchCatalogView, SearchCoreBackendStatus, SearchEngine, SearchHit,
    SearchRequest, Settings, SnippetBatchRequest, SnippetBatchResult, SnippetHit, SnippetRequest,
    UsnCheckpoint, UsnScanResult, INGESTION_VERSION,
};
...
fn start_rebuild(
    ...
    let job_id = format!("portable-{}", now_millis());
    let started_at = now_millis().to_string();
    *state
        .rebuild
        .lock()
                        ...
                        && change_tracker_eligible(&tracker, &previous_catalog, &root, &settings)
                    {
                        ...
                        match engine.scan_changes(
                            ...
                            UsnScanResult::Changes(changes) => {
                                ...
                                match engine.sync_incremental_changes(
        ...
        match result {
            Ok(_catalog) => {
                update_rebuild(&rebuild, |status| {
                    ...
                    status.progress.estimated_completion_at_ms = Some(now_millis() as f64);
                    status.progress.current_path = None;
                    status.finished_at = Some(now_millis().to_string());
                    status.error = None;
                });
            }
