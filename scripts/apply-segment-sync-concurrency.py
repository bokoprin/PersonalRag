...
# Both segment build paths call write_segment directly; update both deterministically.
old = "let written = write_segment(&path, &data, durable)?;"
count = text.count(old)
if count != 2:
    raise SystemExit(f"expected exactly two write_segment calls, found {count}")
text = text.replace(old, "let written = write_segment(&path, &data, durable, sync_limiter)?;")
