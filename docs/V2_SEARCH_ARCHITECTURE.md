...
V2 SHALL retain **exact global trigram-presence semantics**, but SHALL NOT require a fixed 2 MiB dense representation for every corpus.

The persistent representation SHALL be selected adaptively from exact encodings and MUST preserve zero false negatives. The current P0 canonical set is:

- dense 16,777,216-bit bitmap (2 MiB payload),
- sorted observed trigram IDs packed as u24,
- sparse nonzero 64-bit bitmap words addressed by packed u24 word indexes.

The writer SHALL choose the smallest exact representation for the indexed corpus. Readers MUST expose identical membership semantics regardless of encoding.

If any required query trigram is absent globally, a literal query can terminate immediately with zero matches.

P0 evidence on 2026-08-28 showed that this adaptive representation keeps the 96 MiB controlled corpus at approximately 0.793% persistent/source ratio. On a 4 MiB controlled small corpus, adaptive C used approximately 1.678% while the fixed dense representation used approximately 51.659%.

**Decision: FIXED for current V2 P0 unless later evidence invalidates the capacity or latency result.**
...
- adaptive exact global trigram-presence representation from Section 9,
- selected rare-trigram records,
- block postings, preferably compact/delta encoded,
- only the metadata required to locate the selected postings.

The sparse-anchor structures remain subject to the independent <=1.5% selected-source budget. The global-presence representation is selected separately by its exact serialized size.
...
The following are fixed for the current V2 P0 architecture unless new evidence demonstrates that they cannot satisfy the SLOs:
...
| global trigram presence | **adaptive exact representation: dense / packed-u24 / sparse-words** |
