...
public static class RunnerApp
{
    ...
    private static int VerifyLockCommand(string[] args) { if (args.Length < 8) throw new ArgumentException("verify-lock LOCK SPEC GENERATOR ORACLE RUNNER MANIFEST QUERIES REPO_ROOT"); bool valid = LockFile.Verify(args[1], args[2], args[7], args[3], args[4], args[5], args[6]); Console.WriteLine(valid ? "LOCK_PASS" : "LOCK_FAIL"); return valid ? 0 : 1; }
